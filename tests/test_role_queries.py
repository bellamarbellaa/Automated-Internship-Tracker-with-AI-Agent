...
def test_indonesia_queries_covers_exactly_the_three_roles():
    assert set(INDONESIA_QUERIES.keys()) == {"Business Analyst", "Data Analyst", "Consultant"}


def test_indonesia_queries_each_role_has_nonempty_query_list():
    for role, queries in INDONESIA_QUERIES.items():
        assert isinstance(queries, list)
        assert len(queries) > 0
        assert all(isinstance(q, str) and q for q in queries)
